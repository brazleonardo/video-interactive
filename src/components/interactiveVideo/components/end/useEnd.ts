...
export default function useEnd() {
    ...
    const getResultQuiz = useCallback(() => {
        ...
        result.questionsAnswered = contentInteractive.data.filter((item) => 
            item.type === 'quiz' && 
            item.status === 'answered' && 
            item.content.correctAnswer === item.content.questionAnswer).length ?? 0;

        result.total = contentInteractive.data.filter((item) => item.type === 'quiz').length ?? 0;
    ...
    const getResultComment = useCallback(() => {
        ...
        result.questionsAnswered = contentInteractive.data.filter((item) => 
            item.type === 'comment' && 
            item.status === 'answered').length ?? 0;

            result.total = contentInteractive.data.filter((item) => item.type === 'comment').length ?? 0;
    ...
    const getResultCheck = useCallback(() => {
        ...
        result.questionsAnswered = contentInteractive.data.filter((item) => 
            ...
            result.total = contentInteractive.data.filter((item) => item.type === 'check').length ?? 0;
    ...
    const getResultInfo = useCallback(() => {
        ...
        result.questionsAnswered = contentInteractive.data.filter((item) => 
            ...
            result.total = contentInteractive.data.filter((item) => item.type === 'info').length ?? 0;
