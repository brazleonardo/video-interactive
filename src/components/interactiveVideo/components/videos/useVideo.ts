import { useMemo, useRef, useState, useCallback, SyntheticEvent, ChangeEvent } from 'react';
...
export default function useVideo() {
    const { contentInteractive, setContentInteractive, onEnd } = useInteractiveVideo();
    ...
    const compareTimeCurrent = useCallback((timeCurrent: number, time: number) => timeCurrent >= time && timeCurrent <= (time+15), []);

    const updateStateContentInteractive = useCallback((timeCurrent: number) => {
        ...
        setContentInteractive((oldContentInteractive) => {
            ...
            if(hasIteractive){
                ...
                oldContentInteractive.current = oldContentInteractive.data.find((item) => 
                    compareTimeCurrent(timeCurrent, item.time)) as PropsContentInteractive;
                
                hasIteractive = true;
            }
            if(!hasIteractive && !oldContentInteractive.data.find((item) => compareTimeCurrent(timeCurrent, item.time))){
    ...
    const onTimeUpdate = useCallback(() => {
        ...
        sliderRef.current!.style.background = 
            `linear-gradient(to right, ${theme.colors.barVideoSlider} ${progressValue}%, ${theme.colors.secondary} ${progressValue}%)`;
